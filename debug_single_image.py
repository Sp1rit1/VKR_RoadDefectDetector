...
if __name__ == '__main__':
    ...
    IMAGE_TO_DEBUG = "China_Drone_000180.jpg" # Пример имени файла из вашего датасета
    USE_AUGMENTATION = True # Установить True, чтобы включить аугментацию
    AUGMENTATION_SEED = 42 # Установить конкретное число (int) для фиксации аугментации, None для случайной

    # Сколько топ-K якорей каждого типа (позитивные/игнорируемые) визуализировать
    # plot_specific_anchors_on_image теперь не использует top_k параметры,
    # мы их используем только для фильтрации в консольном выводе.
    # Установите их, чтобы контролировать, сколько якорей с IoU >= neg_iou_threshold
    # будет отображено в консоли после сортировки по IoU.
    # TOP_K_POSITIVE = 10 # Эти параметры больше не используются напрямую для визуализации, только для консоли.
    # TOP_K_IGNORED = 10 # Убраны из сигнатуры debug_one_image
    ...
    debug_one_image(
        ...
        # top_k_positive и top_k_ignored удалены из вызова
    )